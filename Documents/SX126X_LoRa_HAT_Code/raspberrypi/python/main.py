...
node = sx126x.sx126x(serial_num = "/dev/ttyAMA0",freq=433,addr=0,power=22,rssi=False,air_speed=2400,relay=False)
...
def image_listener_mode():
    print("[INFO] Modalita ascolto immagini attiva...")
    try:
        while True:
            completed = node.receive_image_chunk()
            if completed:
                print("[SUCCESS] Immagine ricevuta.")
            time.sleep(0.05)
    except KeyboardInterrupt:
        print("\n[INFO] Uscita modalita ascolto immagini.")
...
try:
    ...
    print("Press \033[1;32m3\033[0m   to send an image")
    print("Press \033[1;32m4\033[0m   to recieve an image")
            ...
            if c == '\x73':
                ...
                while True:
                    if sys.stdin.read(1) == '\x63':
                        ...
                        print('\x1b[1A',end='\r')
                        break
                # dectect key '3' per inviare immagine
            if c == '3':
                termios.tcsetattr(sys.stdin, termios.TCSADRAIN, old_settings)  
                image_path = input("Inserisci percorso immagine da inviare:\n> ").strip()
                tty.setcbreak(sys.stdin.fileno())  #
                try:
                    node.send_image(image_path)
                except Exception as e:
                    print(f"[ERROR] Errore nell'invio immagine: {e}")
                break
            if c == '4':
                print("[INFO] Premi Ctrl+C per uscire dalla modalita ascolto immagini")
                image_listener_mode()
